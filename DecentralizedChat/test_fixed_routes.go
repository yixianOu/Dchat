...
func testFixedRoutesCluster() {
	...
	var managers []*routes.NodeManager
	...
	nm1 := routes.NewNodeManager("dchat-cluster", "127.0.0.1")
	...
	nm2 := routes.NewNodeManager("dchat-cluster", "127.0.0.1")
	...
	// 节点3（链式连接测试）
	fmt.Printf("\n📍 步骤4: 添加第三个节点（链式连接）...\n")
	nm3 := routes.NewNodeManager("dchat-cluster", "127.0.0.1")
...
func cleanup(managers []*routes.NodeManager) {
	fmt.Printf("\n🧹 关闭所有节点...\n")
	for i, nm := range managers {
