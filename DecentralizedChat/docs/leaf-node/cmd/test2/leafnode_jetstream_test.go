...
func TestLeafNodeHubAndSpokesWithJetStream(t *testing.T) {
	...
	t.Logf("Hub started (JetStream enabled) - client port: %d, leafnode port: %d", hubOpts.Port, hubOpts.LeafNode.Port)
	...
	t.Logf("Spoke 1 started (JetStream enabled) - client port: %d", spoke1Opts.Port)
	...
	t.Logf("Spoke 2 started (JetStream enabled) - client port: %d", spoke2Opts.Port)
	...
	// ===== 5. JetStream 已通过配置启用 =====
	t.Log("JetStream is enabled on all nodes via configuration")
	...
	t.Log("=== Test with JetStream completed successfully ===")
}
